...
size_t load_identifiers(const char *ids_file, comm_id_array_type **out, const size_t max_n) {
    ...
    temp = (comm_id_array_type *)malloc(sizeof(comm_id_array_type) * max_n);
    if (!temp) {
        fclose(file);
        return 0;
    }
    ...
    for (i = 0; i < max_n && fgets(temp[i], sizeof(comm_id_array_type), file); ++i) {
