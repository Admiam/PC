...
//TODO co je ulozeno v tom vektoru
...
void vector_deinit(struct vector *v){
    ...
    if (v->deallocator)
    {
        for (i = 0; i < vecot_count(v); i++)
        {
            v->deallocator(_vector_at(x,i));
        }
...
int vector_realloc(struct vector *v, const size_t capacity){
    ...
    if (!v || capacity < vector_count(x))
    {
       return 0;
    }
    new_data = remalloc(v->data, capacity * v->item_size);
    if (!new_data)
    {
        return 0;
    }
...
int vector_push_back(struct vector *v, const void *item){
    ...
    memccpy(_vector_at(v, vector_count(v)), item, v->item_size);
    v->count++;
    return 1;
